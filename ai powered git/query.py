...
"""
query_agent.py  —  Agentic Query Engine
========================================
...
Key change: search_files now uses LLM semantic reasoning over the
full file index metadata — no keyword matching, no synonym dicts.
The LLM reads function names, imports, call patterns and decides
which files are relevant based on natural language intent.
    ...
    },
    {
        "toolSpec": {
            ...
            "description": (
                ...
                "The LLM reads the full file index metadata (function names, imports, "
                "call patterns, class names) and returns the most semantically relevant files. "
                "Use this for any 'find where X is handled' or 'which file does Y' question. "
                "Understands intent — e.g. 'rate limiting' will find 'throttle_middleware'."
            ),
            "inputSchema": {
                "json": {
                    "type": "object",
                    "properties": {
                        "query": {
                            ...
                            "description": (
                                ...
                                "'find rate limiting logic', 'which file manages database connections'"
                            )
                        },
                        "branch": {
                            "type": "string",
...
def _llm_search_files(query_text: str, branch: str, branch_data: dict,
                      client, model_id: str) -> str:
    """
    Instead of keyword matching, send the full file index metadata to the LLM
    and ask it to reason about which files are semantically relevant to the query.

    The LLM reads:
      - file paths
      - function names defined in each file
      - class names
      - imports (what the file depends on)
      - outgoing calls (what functions this file calls)
      - commit frequency (hotness signal)

    This means queries like "where is rate limiting handled" will correctly
    find files containing "throttle_middleware", "RateLimiter", etc. because
    the LLM understands semantic intent — not just string overlap.
    """

    # Build compact metadata snapshot — enough signal for the LLM to reason,
    # small enough to fit comfortably in context even for large repos.
    file_summaries = []
    for fp, fe in branch_data["files"].items():
        code  = fe.get("code", {})
        fns   = [_str(f) for f in code.get("functions", [])]
        cls   = [_str(c) for c in code.get("classes", [])]
        imps  = [_str(i) for i in code.get("imports", [])][:12]
        calls = [_str(c) for c in code.get("calls", [])][:10]
    ...
    prompt = f"""You are a code repository analyst. Your job is to find which files in this repository are most relevant to a user's query.

User query: "{query_text}"

Below is the full index of files on branch '{branch}'. For each file you have:
- path: file location in the repo
- functions: function names defined in this file
- classes: class names defined in this file
- imports: what this file imports/depends on
- calls: what external functions this file calls
- commits: how many times this file has been changed (higher = more active)

FILE INDEX:
{json.dumps(file_summaries, indent=2)}

Instructions:
1. Reason about the user's intent — what concept, feature, or behaviour are they asking about?
2. Match that intent against function names, class names, imports and call patterns — NOT just string overlap.
   For example: "rate limiting" should match files with "throttle", "RateLimiter", "slow_down", etc.
3. Return the most relevant files ranked by relevance (most relevant first).
4. Include a clear, specific reason for each file explaining exactly why it matches the query.

Return ONLY a valid JSON array, no markdown fences, no explanation outside the array:
[
  {{"path": "path/to/file.py", "reason": "specific reason why this file is relevant"}},
  ...
]

Return at most 8 results. If nothing is relevant, return an empty array: []"""
    ...
    try:
        ...
        return f"[search_files] Could not parse LLM response:\n{raw}"

    if not results:
        return f"No relevant files found for query: '{query_text}' on branch '{branch}'"

    lines = [f"Semantic search results for: '{query_text}' on branch '{branch}'\n"]
    for r in results:
        fp  = r.get("path", "")
        fe  = branch_data["files"].get(fp, {})
        fns = [_str(f) for f in fe.get("code", {}).get("functions", [])][:6]
        cls = [_str(c) for c in fe.get("code", {}).get("classes", [])][:3]
        lines.append(f"  {fp}")
        lines.append(f"    why       : {r.get('reason', '')}")
        if fns: lines.append(f"    functions : {fns}")
        if cls: lines.append(f"    classes   : {cls}")
        lines.append("")
...
def execute_tool(tool_name: str, tool_input: dict,
                 index: dict, repo_path: str,
                 client=None, model_id: str = "amazon.nova-pro-v1:0") -> str:
    """Execute a tool call requested by Bedrock and return result as string."""
    ...
    # ── search_files — LLM semantic search ───────────────────────────────────
    if tool_name == "search_files":
        ...
        if not query_text:
            return "[search_files] No query provided."

        if not client:
            return "[search_files] No LLM client available for semantic search."

        print(f"    [tool] search_files (LLM semantic): '{query_text}' on {br}")
        return _llm_search_files(query_text, br, branches[br], client, model_id)
...
- List branches and their contents
- Fetch actual source code files
- Search for files by natural language query (semantic search powered by LLM)
- Get commit history, contributors, call graphs
...
1. Always call get_file() to read actual code before explaining what functions do.
   NEVER guess at implementations — fetch the real code first.
2. For branch questions, call get_branch_info() to get the file list.
3. For "how does X work" or "find where X is handled" questions, use search_files() first.
   search_files() understands natural language — pass the full intent as the query,
   e.g. "where is JWT token validation handled" not just "jwt".
4. Be specific — cite actual function names, file paths, commit hashes, and author names.
5. You can call multiple tools in sequence to build a complete answer.
"""
...
def query(user_query: str, index: dict,
    ...
    print(f"\n  Query    : {user_query}")
    print(f"  Mode     : agentic (LLM semantic search + tool calling)")
    ...
    for round_n in range(max_rounds):
        ...
        if stop_reason == "tool_use":
            ...
            for block in msg["content"]:
                ...
                # Pass client + model_id so search_files can make its own LLM call
                result = execute_tool(
                    tool_name, tool_input, index, repo_path,
...
def repl(index: dict, preferred_branch: str | None, aws_region: str, dry_run: bool):
    print("=" * 66)
    print("  AI Git Repo Query — Agentic Mode (LLM Semantic Search)")
    print(f"  Repo  : {index['repository'].get('repo_path', '?')}")
    print(f"  Model : amazon.nova-pro-v1 @ {aws_region}")
    print("  Commands: :branches  :quit")
    print("=" * 66)
...
def main():
    parser = argparse.ArgumentParser(description="Agentic AI git query engine with LLM semantic search.")
    parser.add_argument("-i", "--input-dir", default=".")
    parser.add_argument("--index",     default=None)
    parser.add_argument("-q", "--query", default=None)
    parser.add_argument("--branch",    default=None)
